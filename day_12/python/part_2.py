...
def DFS(node, neighbor_map, visited, path, small_caves):
  path.append(node)
  ...
  if node.islower():
    if node in small_caves.keys():
      ...
      small_caves[node] = 1
  if max(small_caves.values()) >= 2:
    for small_cave in small_caves.keys():
      visited.append(small_cave)
  count = 0
  for neighbor in neighbor_map[node]:
    if neighbor not in visited:
      count += DFS(neighbor, neighbor_map, copy.copy(visited), copy.copy(path), copy.copy(small_caves))
  return count
...
def main(argv):
    ...
    print(DFS('start', neighbor_map, ['start'], [], {"start":0}))
